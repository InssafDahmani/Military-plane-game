...
function checkCollisionWithPlane(obstacle) {
    ...
    if (
        ...
        if (planeHits >= 3) {
            showGameOverModal(); // Show the modal
        }
...
function moveDollar(dollar) {
    const interval = setInterval(() => {
        ...
        // Update the dollar's vertical position using dollarSpeed
        dollar.style.top = `${dollarTop + dollarSpeed}px`;

        // Get the positions of the plane and the dollar
        const planeRect = plane.getBoundingClientRect();
        ...
        if (
            ...
            if (moneySound) {
                moneySound.currentTime = 0; // Reset the sound to start from the beginning
                moneySound.play();
            }
            ...
            // Remove the dollar and clear its interval
            dollar.remove();
            clearInterval(interval);
        }
...
function moveBomb(bomb) {
    const interval = setInterval(() => {
        ...
        if (
            ...
            triggerExplosion(bombRect.left, bombRect.top);
            bomb.remove();
...
function triggerExplosion(x, y) {
    ...
    // Stop the game after explosion animation
    setTimeout(() => {
        showGameOverModal();
        explosion.remove(); // Remove explosion element
    }, 1500); // Match explosion animation duration
}

function showGameOverModal() {
    // Stop any ongoing intervals or game logic
    clearAllIntervals();
    ...
    modal.innerHTML = `
        <h1>Game Over</h1>
        <p>Game over? Don't worry, every great player knows how to hit restart!</p>
...
function shootBullet() {
    ...
    // Position the bullet at the bottom of the plane
    bullet.style.top = `${planeRect.top + planeRect.height / 2 - bannerRect.top}px`;
    bullet.style.left = `${planeRect.left + planeRect.width + 5 - bannerRect.left}px`;
